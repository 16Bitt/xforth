...
forth_env_t* load_new(unsigned int heap_size, unsigned int stack_size){
	...
	dputs("Allocating new forth thread...");
	current = (forth_env_t*) malloc(sizeof(forth_env_t));
	
	dputs("Setting default values...");
	HEAP_SIZE	= heap_size;
	...
	dputs("Allocating heap...");
	HEAP =		(void*) malloc(HEAP_SIZE);
	HERE =		(unsigned int) HEAP;


	dputs("Hard loading native words...");
	hard_load();
	
	dputs("Restoring state...");
